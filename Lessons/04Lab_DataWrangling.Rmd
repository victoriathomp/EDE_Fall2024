...
> Add notes here: All three of these qualities of lakes are particularly 
indicative of lake health and the ability of lakes to support wildlife. 
Long-term monitoring can help reveal overarching trends that can help with 
management. For example, if there is an overall increase in dissolved
oxygen, that indicates an increase in algae, which can encourage landowners to 
target that during management.
...
class(NTL.phys.data.surface$sampledate)
NTL.phys.data.1999 <- filter(NTL.phys.data, year4 == 1999) 

ex1 <- NTL.phys.data %>%  filter (year4 == 1999)


# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tuesday <- filter(NTL.phys.data, lakename %in% 
                                      c("Tuesday Lake"))
NTL.phys.data.Tuesday1999 <-
  filter(NTL.phys.data.Tuesday, year4 <= 1999, year4 >= 1990)
#
#ex2 <- filter(
  #NTL.phys.data, (lakename == 'Tuesday Lake' && year4 %in% c(1990:1999))
)
...
NTL.phys.data.e4 <-NTL.phys.data.e3 %>% 
  filter(depth == 0) %>% 
