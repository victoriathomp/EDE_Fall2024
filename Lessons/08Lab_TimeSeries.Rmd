...
BR_complete_check <-  drop_na(BR_complete)

# no change in values; complete dataset
...
BR_complete_rainfall_ts <- ts(BR_complete$Rainfall, start = c(1901,1), frequency = 12)
BR_complete_temp_ts <- ts(BR_complete$Temperature_C, start = c(1901,1), frequency = 12)
...
BR_Rain_Decomposed <- stl(BR_complete_rainfall_ts, s.window = "periodic")
plot(BR_Rain_Decomposed)

BR_Temp_Decomposed <- stl(BR_complete_temp_ts, s.window = "periodic")
...
BR_Rain_Decomposed_Components <- as.data.frame(BR_Rain_Decomposed$time.series[,1:3])

BR_Rain_Decomposed_Components <- mutate(BR_Rain_Decomposed_Components,
        Observed = BR_complete$Rainfall,     
        Date = BR_complete$Date)


BR_Temp_Decomposed_Components <- as.data.frame(BR_Temp_Decomposed$time.series[,1:3])

BR_Temp_Decomposed_Components <- mutate(BR_Temp_Decomposed_Components,
...
ggplot(BR_Rain_Decomposed_Components, aes (x=Date)) +
  geom_line(aes(y=Observed))+
  geom_line(aes(y=trend), color='red')
 
ggplot(BR_Temp_Decomposed_Components, aes (x=Date)) +
  geom_line(aes(y=Observed))+
  geom_line(aes(y=trend), color='red')
