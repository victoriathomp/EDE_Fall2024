...
airqualfinal <- read.csv(
  file = here('Data','Processed','EPAair_O3_PM25_NC1819_Processed.csv'),
  stringsAsFactors = T
)
airqualfinal$Date <- ymd(airqualfinal$Date)

#split-apply-combine
airqualsummary <- airqualfinal %>%
...
any collumns that have at least one NA. In context, this means that using na.omit
would remove the entire ozone collumn because it contains several NAs.
